...
class Extractor():
    def __init__(self, domain_description) -> None:
        ...
        self.candidates = []
        self.df_concepts = pd.DataFrame(columns=['s_id', 'token', 'position', 'lemmatized_text', 'sent'])
    
    def sentPreprocessing(self, sent):
    ...
    def extract_candidates(self):
        ...
        for sdx,sent in enumerate(sentences):
            sdx= 'S'+str(sdx)
            preprocessed_sent = self.sentPreprocessing(sent)
            self.rule_candidate_concepts(self.language_model(preprocessed_sent),sdx)
        print("\n\n CANDIDATES \n", self.candidates,"\n\n df_concepts \n", self.df_concepts)
    ...
    def rule_candidate_concepts(self, doc,sdx):
        ...
        for chunks in doc.noun_chunks:
            ...
            for chunk_ele in chunk_string:
                if (chunk_ele.shape_ not in no_shape) and ((chunk_ele.tag_ in noun_tags) or ((chunk_ele.tag_ in tag_for_prn_chunks) and chunk_ele.dep_ == 'nsubj') or (chunk_ele.tag_ in verb_tags and chunk_ele.dep_ in comp_deps)): 
                    if chunk_ele.dep_ in ("nmod", "amod","conj","cc","compound"):
                        ...
                        new_row = {'s_id': sdx,'token': chunk_ele,'position': chunk_ele.i, 'lemmatized_text': capitalized_lemmas,'sent': doc.text}
                        new_row_df = pd.DataFrame(new_row, index=[0])
                        self.df_concepts = pd.concat([self.df_concepts, new_row_df], ignore_index=True)  
                        hyphen_flag = False 
                        ...
                        if base_head.pos_ in ("NOUN", "PROPN") and base_head.dep_ not in ("ROOT") and chunk_ele.lemma_ != base_head.lemma_ and not hyphen_flag:
                            base_lemmas = base_head.lemma_
                            base_capitalized_lemmas =  base_lemmas.title()
                            lemm_cadidates.append(''.join(base_capitalized_lemmas))

                            new_row = {'s_id': sdx,'token': base_head,'position': base_head.i, 'lemmatized_text': base_capitalized_lemmas,'sent': doc.text}
                            new_row_df = pd.DataFrame(new_row, index=[0])
                            self.df_concepts = pd.concat([self.df_concepts, new_row_df], ignore_index=True)  
                            rem.append(base_head)
                            m = base_head.head  
                            while(m and m != m.head):
                                if m.pos_ in ("NOUN") and m.dep_ not in ("ROOT") and m.lemma_!= base_head.lemma_ and base_head.dep_ in ("compound","amod","nmod", "cc"):
                                    base_lemmas = m.lemma_
                                    base_capitalized_lemmas =  base_lemmas.title()
                                    lemm_cadidates.append(''.join(base_capitalized_lemmas))
                                    new_row = {'s_id': sdx,'token': m,'position': m.i, 'lemmatized_text': base_capitalized_lemmas,'sent': doc.text}
                                    new_row_df = pd.DataFrame(new_row, index=[0])
                                    self.df_concepts = pd.concat([self.df_concepts, new_row_df], ignore_index=True)                                       
                                    rem.append(m)
        ...
        return self.candidates, self.df_concepts         


if __name__ == "__main__":
    extractor = Extractor("University is composed of departments. Each department has a title.")
